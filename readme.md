...
7. 对于局部路径规划，有一种方法为RRT(rapidly exploring random tree)快速探索随机树作为局部路径规划，他最大的好处是快，快速扩张一群像树一样的路径以探索空间内大部分的区域，是一种对状态空间随机采用的算法，无需预知环境，通过对采样点进行碰撞检测，避免了对空间精确建模带来的巨大计算量，该算法是概率完备但非最优的，并且对于具有狭窄通道和狭窄入口的区域，RRT生长困难，探索效率低下
...
##### multi_mapping.launch实现：

1. 这里使用SLAM建图的算法为Gmapping，SLAM建图后的地图存为静态地图，用于全局路径规划，使用的算法为Dijkstra，而局部路径规划使用的算法为teb_local_planner(常用于智能车)，对Turtlebot3-burger系列，它顶层配置了可以旋转的激光雷达(LIDAR)，由激光雷达内部的电机驱动，当通电时就会开启360度的旋转，并通过检测激光束的反射获得跟周围物体的距离并生成点云数据，在本程序中该激光雷达发布/scan话题同时被SLAM建图节点和局部地图构建订阅，即建图和导航均依赖于激光雷达获得的反射数据，这里需要注意的是amcl蒙特卡洛包同样是基于粒子滤波进行机器人自身位姿的估计，而在Gmapping的过程中同样使用了粒子滤波算法进行位姿估计和建图，故在SLAM的地图构建过程中会停用amcl，这是不必要的，会造成不必要的资源浪费与潜在粒子冲突(不一致的位姿估计)，而如果最后只被要求在静态地图(已知的地图)进行定位，那么会停用Gmapping而启动amcl调用，这里只使用Gmapping
2. 具体而言，在ROS中，启动Gmapping算法包需要订阅/scan，即雷达扫描的数据，而将雷达扫描到的位姿信息最终转换为机器人的基坐标系进行建图，故需要订阅两个/tf话题，一个为机器人当前位置，一个订阅/odom，实际上订阅的是机器人轮子走过的路径被里程计记录的数据，再经过从起点的转换，最后定位到机器人自身的/base_frame，这里我们使用base_footprint作为机器人本体坐标系，而base_footprint和base_link区别：在决定ROS基座标系参数的时候，有base_footprint和base_link，base_link是与机器人底座重合，z轴距离会往上偏移0.1左右，而base_footprint是base_link在地面的投影，在本程序中，需要传入基座标系参数时统一传入base_footprint，还有一个/tf是订阅激光雷达扫描到的数据转换为基坐标系到这些障碍物的距离，会从robot_state_publisher得到激光雷达和基座标系偏差信息，最后读取两个/tf变化一个/scan输入进行算法包的部署
3. 对Gmapping算法的解释：由于在实际环境中从里程计->基坐标系的定位方式存在不准确的情况，比方说轮底打滑等会导致一定的偏差，这个时候会综合根据/scan和/odom数据在机器人可能出现的地方随机分布一定的粒子，每个粒子都带有一副完整的扫描地图和机器人位姿估计，所以Gmapping算法做了两件事情，一个是估计当前机器人的位姿(位置+姿态信息)，一个是估计了当前扫描的栅格化地图信息，而为了解决二者先后顺序，RBpf算法(Gmapping前身)决定了先得到估计位姿，后根据雷达信息进行地图构建，由于每个粒子都带有一份地图(已扫描区域和未扫描区域)，当粒子数量适中但是场景过大达到千米的数量级会导致计算内存崩溃，
